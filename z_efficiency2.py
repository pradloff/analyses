...
class efficiency_weight(event_function):
	...
	def initialize_tools(self):
		analysis_home = os.getenv('ANALYSISHOME')
		try: file_name = '{0}/data/{1}_efficiency_alt2.root'.format(analysis_home,{0:'ee',1:'mumu',2:'emu'}[self.lepton_class])
...
class efficiency(result_function):
	...
	def __init__(
		...
		if lepton_class in [0,1]:
			...
			pts = [
				10.+2.*i for i in range(15)
				]+[\
				40.+5.*i for i in range(5)
				]+[\
				1000.,
				]
...
class identify_z_leptons(event_function):
	...
	class z_not_found(EventBreak): pass
	class ee_not_found(EventBreak): pass
	class mumu_not_found(EventBreak): pass
	class tautau_not_found(EventBreak): pass
