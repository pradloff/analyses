...
void fitTopHists(TString TaggerName = "SV0",
		 TString OPName = "5_85",
		 TString OutFileName = "TopCalibrations_rel16_final_2010.root",
		 ...
		 bool prelim = false)
{
  ...
  TFile *file_rel16 = new TFile("mc10merge.root", "read");
  ...
  if (OPName == "2_05")
    lFitFormula = "[0] + [1]*log(x[1]) + [2]*abs(x[0]) + [3]*pow(log(x[1]),2.0) + [4]*pow(abs(x[0]),2.0) + [5]*x[1]*abs(x[0]) + [6]*pow(log(x[1]),4.0) + [7]*pow(abs(x[0]),4.0) + [8]*pow(log(x[1])*abs(x[0]),2.0)";
  ...
  if (doDisplay) {
    ...
    //    hlight -> GetZaxis() -> SetRangeUser(0., 0.12);
    /*
    if (OPName.Contains("1_40"))
      hlight -> GetZaxis() -> SetRangeUser(0., 0.25);
    if (OPName.Contains("0_60"))
      hlight -> GetZaxis() -> SetRangeUser(0., 0.50);
    */
