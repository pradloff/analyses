...
TH2* MakeCustomRatio(TH2* hnum, TH2* hdenom) 
{
  ...
  for(int bxi = 1; bxi <= heff->GetNbinsX(); bxi++) {
	...
	cout << "Removing negative bin " << bxi << "," << byi << endl;
...
bool GetxIsEta(TH2 *hb)
{
  if (hb)
    return (hb -> GetXaxis() -> GetXmax() - hb -> GetXaxis() -> GetXmin()) < 100.;
  return 0;
}

bool GetyIsPt(TH2 *hb)
{
  if (hb)
    return (hb -> GetYaxis() -> GetXmax() - hb -> GetYaxis() -> GetXmin()) > 100.;
  return 0;

}

bool GetyIsAbsEta(TH2 *hb)
{
  if (hb)
    return hb -> GetYaxis() -> GetXmin() > -1.e-3;
  return 0;
}

bool GetxIsAbsEta(TH2 *hb)
{
  if (hb)
    return hb -> GetXaxis() -> GetXmin() > -1.e-3;
  return 0;
}

// _______________________________________________________________________________
void CheckHistoRanges(TH2* hb)
{
  bool xIsEta = GetxIsEta(hb);
  bool yIsPt = GetyIsPt(hb);
  bool xIsAbsEta =  GetxIsAbsEta(hb);
  cout << " xmin=" << hb -> GetXaxis() -> GetXmin()
       << " xmax=" << hb -> GetXaxis() -> GetXmax()
       << " ymin=" << hb -> GetYaxis() -> GetXmin()
       << " ymax=" << hb -> GetYaxis() -> GetXmax()
       << endl;
  cout << " We think that: "
       << " xIsEta=" << xIsEta
       << " yIsPt=" << yIsPt
...
TLatex *MakeText(TF2 *fun)
{
  
  if (!fun)
    return 0;

  TLatex *text = new TLatex(0.15, 0.85, Form("#chi^{2}/NDF: %5.3f", fun -> GetChisquare() / fun -> GetNDF()));
  text -> SetNDC();
