...
#include "share/InSituSystematics.C"
#include <TSystem.h>

JERProvider::JERProvider(std::string CollectionName, std::string MethodName, std::string FileName):
  m_collectionName(CollectionName), m_methodName(MethodName), m_fileName(FileName), m_isInit(false),m_GeV(1000),m_is7TeV(false)
{
...
double JERProvider::getRelResolutionMC(double pt, double eta) {
  ...
  if (pt*cosh(eta)>10000.0*m_GeV) 
    fatal(Form("I refuse! You are giving me a jet with E=%.2f TeV. Can't be right. Check your units. OR this might be new physics!",
	       pt*cosh(eta)/m_GeV/1000));
  if (pt*cosh(eta)>4200.0*m_GeV) warning(Form("You have a jet with suspisiosly large energy E= %.2f TeV.",pt*cosh(eta)/m_GeV/1000));
  if (pt/m_GeV < 10 ) pt=10.0*m_GeV; if (pt/m_GeV > 2000 ) pt=2000.0*m_GeV;
  return m_jerFunc.at(getEtaBin(eta))->Eval(pt/m_GeV);
}
...
double JERProvider::getRelResolutionMC_AFII(double pt, double eta) {
  ...
  if (pt*cosh(eta)>10000.0*m_GeV) 
    fatal(Form("I refuse! You are giving me a jet with E=%.2f TeV. Can't be right. Check your units. OR this might be new physics!",
	       pt*cosh(eta)/m_GeV/1000));
  if (pt*cosh(eta)>4200.0*m_GeV) warning(Form("You are using a large jet: E=%.2f TeV.",pt*cosh(eta)/m_GeV/1000));

  if (pt/m_GeV < 10 ) pt=10.0*m_GeV; if (pt/m_GeV > 2000 ) pt=2000.0*m_GeV;
  return m_jerFuncAFII.at(getEtaBin(eta))->Eval(pt/m_GeV);
}
...
double JERProvider::getInsituRelResolutionData(double pt, double eta) {
  ...
  return m_jerData[etaBin]->Eval(pt/m_GeV);
}
...
double JERProvider::getInsituRelResolutionMC(double pt, double eta) {
  ...
  if (etaBin>3) etaBin=3; // due to lacking of stats from data/mc
  if (pt>1000*m_GeV) pt=1000*m_GeV; if (fabs(eta)>2.1 && pt>300*m_GeV) pt=300*m_GeV;
  return m_jerMC[etaBin]->Eval(pt/m_GeV);
}
...
double JERProvider::getInsituUncertData(double pt, double eta) {
  ...
  if (etaBin>3) etaBin=3; // due to lacking of stats from data/mc
  if (pt>1000*m_GeV) pt=1000*m_GeV; if (fabs(eta)>2.1 && pt>300*m_GeV) pt=300*m_GeV;
  return m_errorData[etaBin]->Eval(pt/m_GeV);
}
...
double JERProvider::getInsituUncertMC(double pt, double eta) {
  ...
  if (etaBin>3) etaBin=3; // due to lacking of stats from data/mc
  if (pt>1000*m_GeV) pt=1000*m_GeV; if (fabs(eta)>2.1 && pt>300*m_GeV) pt=300*m_GeV;
  return m_errorMC[etaBin]->Eval(pt/m_GeV);
}
...
double JERProvider::getInsituDiffDataMC(double pt, double eta) {
  ...
  if (pt>1000*m_GeV) pt=1000*m_GeV; if (fabs(eta)>2.1 && pt>300*m_GeV) pt=300*m_GeV;
  return m_diffDataMC[etaBin]->Eval(pt/m_GeV);
}
double JERProvider::getInsituDiffDataMCerror(double pt, double eta) {
  ...
  return m_errorDataMC[etaBin]->Eval(m_pt/m_GeV)/100;
}
...
double JERProvider::getUncertainty_7TeV(double pt, double eta) {
  ...
  float factor = 0;
  if ( pt/m_GeV >= ptmin && pt/m_GeV <= ptmax) factor = 0;
  else if( pt/m_GeV > ptmax && pt/m_GeV <= ptpiv) factor = (pt/m_GeV - ptmax)/(ptpiv-ptmax);
  else if( pt/m_GeV > ptpiv) factor = 1;
...
double JERProvider::getDeltaInSitu(double pt, double y)
{
  ...
  mySyst.GetSRIsystematic(m_collectionName, pt/m_GeV, y, InSituSyst);
