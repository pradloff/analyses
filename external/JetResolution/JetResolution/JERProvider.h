...
class JERProvider : public TNamed
{
 ...
 public: 
  ...
  JERProvider() { }
  JERProvider(std::string CollectionName, std::string MethodName = "Truth", std::string FileName="$ROOTCOREDIR/data/JetResolution/JERProviderPlots_2012.root");
  virtual ~JERProvider();
  ...
  //initialization
  void init();
  void useGeV(bool isGeV=true) { if (isGeV) m_GeV = 1; else m_GeV = 1000;}
