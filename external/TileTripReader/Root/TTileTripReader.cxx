...
//#ifndef __STANDALONE__
//const TAccept& TTileTripReader::accept(int run, int lbn, unsigned int event, double eta, double phi){
...
//#ifndef __STANDALONE__
//const TResult& TTileTripReader::calculate(int run, int lbn, double eta, double phi, double dR){
...
bool TTileTripReader::checkEtaPhi(int run, int lbn, double eta, double phi){
    ...
    for(size_t i=0;i<ntrips;++i){
        if(tripRegions[i].eta1<=eta && tripRegions[i].eta2>=eta &&
               tripRegions[i].phi1<=phi && tripRegions[i].phi2>=phi ) 
            return true;
    }
    return false;
}
...
vector<TripRegion> TTileTripReader::getEtaPhi(int run, int lbn){
    ostream& msg=*m_msglog;
    if(run==m_currentRun && m_currentLbn==lbn){
        if(m_verbosity==Debug){
    ...
    if(m_verbosity==Debug)
        msg<<"Trip regions found for run "<<run<<" lbn "<<lbn<<
                endl;
    do{
        ...
        if(lbn<m_LumiStart)return tripRegions;
        vector<char>& part=*m_Partition;
        vector<char>& mod=*m_Module;
        vector<int>& lbend=*m_LumiEnd;
        size_t ntrips=lbend.size();
        ...
        ++entry;
    }while(m_currentRun==m_Run);
    m_currentTrips=tripRegions;
    m_currentLbn=lbn;
    return tripRegions;
}
