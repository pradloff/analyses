...
int main() {
  ...
  TString pdf="JES_Flavor_2012_Njets.pdf";
  //from 2011
  TString mJES="../testingMacros/MultijetJES_Preliminary_Njets.config";
  //from 2012
  TString JESConfig="JESUncertainty2012_FirstData.config";
  TString MCTag = "Pythia8";
  TString thePath="../share/";
  //pdf="JES_Flavor_KnownFlavComp.pdf";
  //mJES="MultijetJES_KnownFlavComp.config";
  ...
  //log binning
  int N = 100; double min = 15, max = 2500;
  ...
  for (int ji=0;ji<4;++ji) {
    //CATE'S HACK: was 4:6
    int jetR = ji%2==0?4:4;
    //CATE'S HACK: was EM:LC
    TString constScale = ji<2?"EM":"EM";
...
//     MultijetJESUncertaintyProvider j(mJES,"InsituJES2011_AllNuisanceParameters.config",jetAlgo,"FrozenShowers")
  ...
  for (int bin=1;bin<=h_jes->GetNbinsX();++bin) {
    ...
    h_fComp->SetBinContent(bin,(j.getRelFlavorCompUncert(pt,eta,true,2)+j.getRelFlavorCompUncert(pt,eta,false,2))/2);
    h_fResp->SetBinContent(bin,j.getRelFlavorResponseUncert(pt,eta,2));
