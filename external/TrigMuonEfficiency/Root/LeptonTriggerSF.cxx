                              ...
                              const std::vector< ElectronQuality >& electronsQuality) const
{
  ...
  for (size_t ielec = 0; ielec < electrons.size(); ++ielec) {
    ...
    const int set_data = getElectronQuality(configuration.runNumber, electronsQuality.at(ielec), true, false);
    const int set_mc   = getElectronQuality(configuration.runNumber, electronsQuality.at(ielec), false, false);
                              ...
                              const std::string& trigger) const
{
  ...
  const bool is_e24vi_medium1_or_e60_medium1 = (trigger.find("e24vi_medium1_or_e60_medium1") != std::string::npos);
  const bool is_2e12Tvh_loose1 = (trigger.find("2e12Tvh_loose1") != std::string::npos);

  // get efficiency from data
  configuration.isData = true;
  if ((is_e24vi_medium1_or_e60_medium1 and is_2e12Tvh_loose1)) { // single OR dielectron
                                     ...
                                     const std::string& chain) const
{
  ...
  const int set_data_leg1 = getElectronQuality(configuration.runNumber, electronsQuality.at(Leg1), true, true);
  const int set_mc_leg1   = getElectronQuality(configuration.runNumber, electronsQuality.at(Leg1), false, true);
  const int set_data_leg2 = getElectronQuality(configuration.runNumber, electronsQuality.at(Leg2), true, true);
  const int set_mc_leg2   = getElectronQuality(configuration.runNumber, electronsQuality.at(Leg2), false, true);
                                             ...
                                             const std::string& chain) const
{
  ...
  if (chain.find("mu18_tight_mu8_EFFS") != std::string::npos ){
    ...
    const std::pair<double, double> mu1_pair = getMuonEfficiency(config, muons.at(0), muonsQuality.at(0), "mu13");
    const std::pair<double, double> mu2_pair = getMuonEfficiency(config, muons.at(1), muonsQuality.at(1), "mu13");
                                             ...
                                             const std::string& chain) const
{
  ...
  const int set_data_el1 = getElectronQuality(config.runNumber, electronsQuality.at(0), true, false);
  const int set_mc_el1   = getElectronQuality(config.runNumber, electronsQuality.at(0), false, false);
  const int set_data_el2 = getElectronQuality(config.runNumber, electronsQuality.at(1), true, false);
  const int set_mc_el2   = getElectronQuality(config.runNumber, electronsQuality.at(1), false, false);
  double el1_eff = 0.;
  ...
  const bool is_el1_above_threshold = ((electrons.at(0)).Pt() >= threshold.first);
  const bool is_el2_above_threshold = ((electrons.at(1)).Pt() >= threshold.first);

  const int set_data_el1_low = getElectronQuality(config.runNumber, electronsQuality.at(0), true, true);
  const int set_mc_el1_low   = getElectronQuality(config.runNumber, electronsQuality.at(0), false, true);
  const int set_data_el2_low = getElectronQuality(config.runNumber, electronsQuality.at(1), true, true);
  const int set_mc_el2_low   = getElectronQuality(config.runNumber, electronsQuality.at(1), false, true);
...
{
  const double T1 = ( ( (1-b1) * a2 ) * ( (1-b1) * a2 ) ) + ( ( (1-a1) * b2 ) * ( (1-a1) * b2 ) );
