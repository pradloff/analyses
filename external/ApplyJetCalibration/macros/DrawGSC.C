...
/*******************/
/***Global option**/
bool _plotresponse=false; //set to true if you want to plot the correction instead of the response!
...
Str GetEta(double eta) { return Form("#eta = %.1f",eta); }
...
int main(int argc, char** argv) {
  ...
  for(int ialgo=0; ialgo<jetAlgos.size(); ++ialgo) {
    ...
    for(int ieta=0; ieta<Neta; ++ieta) {
      ...
      DrawGSC_vs_trackWIDTH(jetAlgos[ialgo], etabins[ieta]/10);
      DrawGSC_vs_nTrk(jetAlgos[ialgo], etabins[ieta]/10);
      DrawGSC_vs_Tile0(jetAlgos[ialgo], etabins[ieta]/10);
      DrawGSC_vs_EM3(jetAlgos[ialgo], etabins[ieta]/10);
      Can->Print(ps);
    }
  }
...
void DrawGSC_vs_trackWIDTH(Str jetAlgo, double eta) {
  ...
  h->SetMaximum(1.12); h->SetMinimum(0.9);
