...
void SmearingClass::SetAlgoSmearRelDir(std::string Data,
  ...
  if(Algo=="muid" || Algo=="staco" || Algo=="muons"){
    ...
    std::cerr<<"SmearingClass ERROR:: wrong Algorithm in input "<<Algo<<std::endl;
  ...
  if(SmearType=="pT") {
    ...
    std::cerr<<"SmearingClass ERROR:: wrong Smear Type in input "<<SmearType<<std::endl;
    std::cout<<"                      possible values are: "<<std::endl;
  ...
  if(Rel=="Rel16.6" || Rel=="Rel17" || Rel=="Rel17.2" || Rel == "Rel17.2Repro"){
    ...
    std::cerr<<"SmearingClass ERROR:: wrong Release in input "<<Rel<<std::endl;
    std::cout<<"                      possible values are: "<<std::endl;
  ...
  if(m_Algorithm=="muons" && m_DataYear!="Data12"){
    std::cerr<<"SmearingClass ERROR:: wrong settings:"<<std::endl;
...
void SmearingClass::SetRegions(){
  ...
  if (m_Release == "Rel17.2Repro") {
    std::string regions = m_Fdir+"RegionsRepro.dat";
  }
...
void SmearingClass::FillValues(){
  ...
  i=0;
  if(!InValues.good()){
    std::cerr<<"SmearingClass ERROR:: File "<<scale_val<<" NOT FOUND!!"<<std::endl;
    std::cout<<"                      Please provide a correct path!"<<std::endl;
    return;
  }
  else{
  ...
  i=0;
  if(!InValues.good()){
    std::cerr<<"SmearingClass ERROR:: File "<<mc_val<<" NOT FOUND!!"<<std::endl;
    std::cout<<"                      Please provide a correct path!"<<std::endl;
    return;
  }
  else{
  ...
  i=0;
  if(!InValues.good()){
    std::cerr<<"SmearingClass ERROR:: File "<<corr_val<<" NOT FOUND!!"<<std::endl;
    std::cout<<"                      Please provide a correct path!"<<std::endl;
    return;
  }
  else{
...
void SmearingClass::FillScales(std::string Scaletype, bool ApplyScalesToData){
  ...
  if(m_ScaleType==""     || m_ScaleType=="default" || 
    ...
    std::cerr<<"SmearingClass ERROR:: wrong Scale Type in input "<<m_ScaleType<<std::endl;
...
void SmearingClass::Event(double Pt,
  ...
  else std::cerr<<"SmearingClass ERROR: wrong DetType in input "<<DetType<<std::endl;
...
void SmearingClass::Event(double PtMS,
  ...
  ptcb=0;
  if(PtMS<=0 && PtID>0){
    std::cout<<"SmearingClass WARNING:: pTMS <=0 ..."
    ...
    std::cout<<"SmearingClass WARNING:: pTID <=0 ..."
    ...
    std::cout<<"SmearingClass ERROR:: both pTID & pTMS <=0 ..."
...
void SmearingClass::Event() {
  ...
  smearMS=Smearing(DET_MS);
  smearID=Smearing(DET_ID);
  if (Tdet==DET_CB){ 
    smearCB=ICombine(smearMS,smearID);
  }
  ErrorMatrix();
}
...
double SmearingClass::Smearing(int DetType) {
  ...
  if (DetType==DET_MS){
    ...
    std::cerr<<"SmearingClass ERROR:: DetType not defined "<<DetType<<std::endl;
  }
  return 0;
}
...
double SmearingClass::ICombine(double SmearMS, 
  ...
  if(!useImprovedCombine) 
    return Combine(SmearMS,SmearID);
  else 
    return Combine2(SmearMS,SmearID);
...
double SmearingClass::ExpectedResolution(std::string DetType, bool mc) const {
  ...
  if(DetType=="MS"){ 
    ...
    std::cerr<<"SmearingClass ERROR: wrong DetType in input "<<DetType<<std::endl;
    return 0.;
  }
...
double SmearingClass::ScaleCB(double pt) const{
  ...
  if(m_ScaleType!=""){
    if(unsigned(scaleRegion)>=m_S1_CB.size() || unsigned(scaleRegion)>=m_S2_CB.size()){
      std::cerr<<"SmearingClass ERROR:: "<<scaleRegion<<" length of scaleRegion1: "<<m_S1_CB.size()<<std::endl;
      std::cerr<<"SmearingClass ERROR:: "<<scaleRegion<<" length of scaleRegion2: "<<m_S2_CB.size()<<std::endl;
      return -1;
    }
  }
...
double SmearingClass::ScaleApply(double pt, double S1, double S2, double S) const{
  ...
  if(Tscale==SCALE_KPKM) {
    if(charge==0) {
      std::cerr <<"ERROR: must provide muon charge via Event() to use advanced pT scaling" << std::endl;
      return pt;
    }
...
SYSTTYPE SmearingClass::SelectFun(std::string fun){
  ...
  else if(fun=="SCALEKUP") { Tfun=SCALEK_UP; }
  else if(fun=="SCALEKDOWN") { Tfun=SCALEK_DOWN; }
  else if(fun=="SCALECUP") { Tfun=SCALEC_UP; }
  else if(fun=="SCALECDOWN") { Tfun=SCALEC_DOWN; }
  return Tfun;
}
...
void SmearingClass::PTVar(double &PtMS,
  ...
  else if (Tfun==ID_LOW) { IDLOW(PtMS,PtID,PtCB); return; }
  else if (Tfun==MS_UP) { MSUP(PtMS,PtID,PtCB); return; }
  else if (Tfun==MS_LOW) { MSLOW(PtMS,PtID,PtCB); return; }
  else if (Tfun==SCALEK_UP || Tfun==SCALEK_DOWN) {
    const double s = (fun=="SCALEKUP") ? 1.0 : -1.0;
    
    if(Tscale==SCALE_KC || Tscale==SCALE_K) {
      // here, we scale up or down the overall pT scale
      PtMS = ScaleApply(pTMS(smearMS,0),m_S1_MS[scaleRegion] + s*m_S1Corr_MS[scaleRegion],m_S2_MS[scaleRegion]);
      PtID = ScaleApply(pTID(smearID,0),m_S1_ID[scaleRegion] + s*m_S1Corr_ID[scaleRegion],m_S2_ID[scaleRegion]);
      PtCB = ScaleApply(pTCB(smearCB,0),m_S1_CB[scaleRegion] + s*m_S1Corr_CB[scaleRegion],m_S2_CB[scaleRegion]);
      return;
    }
    
    else if(Tscale==SCALE_KPKM) {
      // here, we scale up or down the component of kp/km error from Z mass fit
      PtMS = ScaleApply(pTMS(smearMS,0),m_S1_MS[scaleRegion] + s*m_S1Corr_MS[scaleRegion],m_S2_MS[scaleRegion] - s*m_S2Corr_MS[scaleRegion]);
      PtID = ScaleApply(pTID(smearID,0),m_S1_ID[scaleRegion] + s*m_S1Corr_ID[scaleRegion],m_S2_ID[scaleRegion] - s*m_S2Corr_ID[scaleRegion]);
      PtCB = ScaleApply(pTCB(smearCB,0),m_S1_CB[scaleRegion] + s*m_S1Corr_CB[scaleRegion],m_S2_CB[scaleRegion] - s*m_S2Corr_CB[scaleRegion]);
      return;
    }
    
    else if(Tscale==SCALE_DEFAULT) {
      const double SCB = (detRegion<0 || detRegion>=m_nb_regions) ? 1.0 : m_scale_CB[detRegion] + s*m_scaleSyst_CB[detRegion];
      PtCB = ScaleApply(pTCB(smearCB,0),0,0,SCB);
      return;
    }
  ...
  else if (Tfun==SCALEC_UP || Tfun==SCALEC_DOWN) {
    const double s = (fun=="SCALECUP") ? 1.0 : -1.0;
    if(Tscale==SCALE_KC || Tscale==SCALE_K) {
      // here, we scale up or down the curvature offset
      PtMS = ScaleApply(pTMS(smearMS,0),m_S1_MS[scaleRegion],m_S2_MS[scaleRegion] + s*m_S2Corr_MS[scaleRegion]);
      PtID = ScaleApply(pTID(smearID,0),m_S1_ID[scaleRegion],m_S2_ID[scaleRegion] + s*m_S2Corr_ID[scaleRegion]);
      PtCB = ScaleApply(pTCB(smearCB,0),m_S1_CB[scaleRegion],m_S2_CB[scaleRegion] + s*m_S2Corr_CB[scaleRegion]);
      return;
    }
    
    else if(Tscale==SCALE_KPKM) {
      // here, we scale up or down the component of kp/km error from R0 fit
      PtMS = ScaleApply(pTMS(smearMS,0),m_S1_MS[scaleRegion] + s*m_S1ACorr_MS[scaleRegion],m_S2_MS[scaleRegion] + s*m_S2ACorr_MS[scaleRegion]);
      PtID = ScaleApply(pTID(smearID,0),m_S1_ID[scaleRegion] + s*m_S1ACorr_ID[scaleRegion],m_S2_ID[scaleRegion] + s*m_S2ACorr_ID[scaleRegion]);
      PtCB = ScaleApply(pTCB(smearCB,0),m_S1_CB[scaleRegion] + s*m_S1ACorr_CB[scaleRegion],m_S2_CB[scaleRegion] + s*m_S2ACorr_CB[scaleRegion]);
      return;
    }
