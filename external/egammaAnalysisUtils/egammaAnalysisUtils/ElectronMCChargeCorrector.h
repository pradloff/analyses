...
public:
  enum eChargeCorrection {chg_nominal, chg_sysup, chg_sysdown};
  ...
  int GetCorrectedCharge(double etaclus_rec, int charge_rec, int charge_true, eChargeCorrection type = chg_nominal) const;

  int GetCorrectedCharge(double etaclus_rec, int charge_rec, 
			 const TLorentzVector & el_fourv_rec,
			 ...
			 eChargeCorrection type = chg_nominal) const
  {
    return GetCorrectedCharge(etaclus_rec, charge_rec,
			      GetTruthCharge(el_fourv_rec, el_fourv_true, el_charge_true),
			      type);
  }
