...
vector<FsrPhotons::FsrCandidate>* FsrPhotons::getFsrPhotons(float mueta, float muphi, float muidtheta, float muidphi,  vector<float> *pheta, vector<float> *phphi, vector<float> *phEt, vector<float> *phf1, vector<int> *phauthor, vector<float> *eleta, vector<float> *elphi, vector<float> *elEt, vector<float> *elf1, vector<float>* eltracktheta, vector<float>* eltrackphi, double DRCUT, double ETCUT, double F1CUT, std::string option) {
  ...
  // sort FSR candidates w.r.t min dR
  if (option=="ET") std::sort(cand->begin(), cand->end(), compareEt);
  else std::sort(cand->begin(), cand->end());
