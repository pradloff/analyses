...
ElectronMCChargeCorrector::ElectronMCChargeCorrector(std::string sim_id, std::string fileName)
{
  ...
  if (weightfile == NULL) {
    ...
    if (chMID_data == NULL || chMID_mc == NULL || corr == NULL) {
      Error("ElectronMCChargeCorrector::ElectronMCChargeCorrector", "Histogram: chmid_data or chmid_mc or corr not found");
    } else {
...
{
  if (chMID_data) delete chMID_data;
  if (chMID_mc) delete chMID_mc;
  if (corr) delete corr;
}

//------------------------------------------------------
int ElectronMCChargeCorrector::GetCorrectedCharge(double etaclus_rec, int charge_rec,
						  int charge_true, eChargeCorrection type) const
{
  if (chMID_data == NULL || chMID_mc == NULL || corr == NULL) {
    return charge_rec;
  }
  ...
  if (val_mc > val_data 
      && (charge_rec != charge_true) 
      && rmd < ((val_mc-val_data)/val_mc) ) {
    return charge_true;
  } else if (val_mc < val_data
	     && (charge_rec == charge_true)
	     && rmd < ((val_data-val_mc)/(1-val_mc)) ) {
    return -charge_rec;
  } else {
    return charge_rec;
  }
...
//------------------------------------------------------
int ElectronMCChargeCorrector::GetTruthCharge(const TLorentzVector & el_fourv_rec,
					      const std::vector<TLorentzVector> & el_fourv_true,
